...
impl RedisQueue {
    pub async fn new(
        ...
        Ok(Self { client, source_queue: source.to_string(), processing_queue: processing.to_string(), semantics })
    }
}
...
impl TaskQueue for RedisQueue {
    ...
    async fn ack(&self, task_id: &str) -> Result<(), QueueError> {
        ...
        let _: () = conn.lrem(&self.processing_queue, 1, task_id)
            .await
            .map_err(|e| QueueError::Internal(e.to_string()))?;
        Ok(())
    }
    ...
    async fn nack(&self, task_id: &str) -> Result<(), QueueError> {
        ...
        let mut conn = self.client.get_tokio_connection()
            .await
            .map_err(|e| QueueError::Internal(format!("redis conn: {}", e)))?;
        // Возвращаем задачу в основную очередь и удаляем из processing
        let _: () = conn.lpush(&self.source_queue, task_id)
            .await
            .map_err(|e| QueueError::Internal(e.to_string()))?;
        let _: () = conn.lrem(&self.processing_queue, 1, task_id)
            .await
